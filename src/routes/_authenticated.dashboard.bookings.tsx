...
function statusColor(s: string) {
  ...
  if (s === "pending_cash") return "bg-[color:var(--warning)]/15 text-[color:var(--warning)] border-transparent";
        ...
        <DialogContent>
          ...
          {selected && (
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between"><span className="text-muted-foreground">Status</span><Badge variant="outline" className={statusColor(selected.status)}>{statusLabel(selected.status)}</Badge></div>
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Payment</span>
                <span className="flex items-center gap-1.5 font-medium">
                  {selected.payment_method === "cash"
                    ? <><Banknote className="h-3.5 w-3.5 text-[color:var(--warning)]" /> Cash at appointment</>
                    : <><CreditCard className="h-3.5 w-3.5 text-primary" /> Stripe deposit</>}
                </span>
              </div>
              ...
              {selected.discount_id && discountById[selected.discount_id] && (
                <div className="flex items-start gap-2 p-2 rounded-lg bg-[color:var(--success)]/10 border border-[color:var(--success)]/30 text-xs">
                  <Percent className="h-3.5 w-3.5 mt-0.5 text-[color:var(--success)]" />
                  <span><span className="font-medium text-[color:var(--success)]">Discount applied:</span> {discountById[selected.discount_id].reason || "—"} — −{formatPrice(selected.discount_applied ?? 0)}</span>
                </div>
              )}
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {selected.status === "pending_cash" && (
                  <Button size="sm" className="bg-gradient-primary glow-primary" onClick={() => setStatus(selected, "completed")}>
